...
### 关于Unity版本
- 因为Unity2019.3之后的Gradle目录结构有所变化，所以暂不支持Unity2019.3之前的版本[Gradle项目结构](https://docs.unity3d.com/Manual/android-gradle-overview.html)
...
#### 如果您使用了 自定义 AndroidManifest.xml
##### 调整 AndroidManifest 的 application
```
...
##### 补充以下文本到你的 AndroidManifest 的 activity 下方(不是activity内)
```
<!-- END -->
<!-- Android 9以上适配-->
...
``` 
    <application android:extractNativeLibs="true" android:label="@string/app_name" android:icon="@mipmap/app_icon"
             android:networkSecurityConfig="@xml/network_security_config" android:fullBackupContent="@xml/custom_backup_rule" tools:replace="android:fullBackupContent"/>
```

#### 如果您使用了 自定义 baseProjectTemplate.gradle
##### 国内版本请新增以下内容
```
buildscript {
